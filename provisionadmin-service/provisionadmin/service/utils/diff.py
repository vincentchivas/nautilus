...
def diff_locale_origin(locale_dir, origin_dir):
    ...
    for xml_file in origin_xml_files:
        o_xml_path = os.path.join(
            os.path.join(origin_path, origin_dir), xml_file)
        o_xml_tree = ET.ElementTree(file=o_xml_path)
        o_xml_root = o_xml_tree.getroot()
        l_xml_path = os.path.join(
            os.path.join(origin_path, locale_dir), xml_file)
        missing_filename = "%s_missing%s" % tuple(os.path.splitext(xml_file))
        miss_xml_path = os.path.join(
            os.path.join(miss_path, locale_dir), missing_filename)
        remove_list = []
...
def diff_xml(o_path, m_path):
    ...
    locale_values_dirs = filter(
        lambda s: s.startswith('values-'), os.listdir(origin_path))
    '''
    check miss dir
    '''
