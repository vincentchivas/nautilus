...
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/var/app/enabled/cm_console
DAEMON=/usr/local/bin/uwsgi
NAME=CM-Console
DESC="common Console Service uWSGI Web Server"
PROJECT=cm_console_service
SPIDERS=1
...
function start_uwsgi()
{
	if [ -f "$PID_FILE" ]; then
	    ...
	    pushd ${APP_DIR}/cm_console >/dev/null
