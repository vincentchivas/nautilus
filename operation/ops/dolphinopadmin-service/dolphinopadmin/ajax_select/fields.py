        ...
        if value:
            ...
            if len(objs) != 1:
                ...
                raise forms.ValidationError(
                    u"%s cannot find object: %s" % (lookup, value))
            return objs[0]
        else:
            if self.required:
...
class AutoCompleteSelectMultipleWidget(forms.widgets.SelectMultiple):
    ...
    def render(self, name, value, attrs=None):
        ...
        if value:
            current_ids = "|" + "|".join(str(pk)
                                         for pk in value) + "|"  # |pk|pk| of current
        else:
            current_ids = "|"
    ...
    def __init__(self, channel, *args, **kwargs):
        ...
        if not (help_text is None):
            ...
            if help_text != u'':
                ...
                django_default_help = _(u'Hold down "Control", or "Command" on a Mac, to select more than one.').translate(
...
def bootstrap():
    ...
    if inlines == 'inline':
        ...
        js = f.read()
        b['inline'] = mark_safe(
            u"""<style type="text/css">%s</style><script type="text/javascript">//<![CDATA[%s//]]></script>""" % (css, js))
    elif inlines == 'staticfiles':
        b['inline'] = mark_safe("""<style type="text/css">@import url("%sajax_select/css/ajax_select.css");</style><script type="text/javascript" src="%sajax_select/js/ajax_select.js"></script>""" %
