...
logger = logging.getLogger('dolphinopadmin.base')
...
def resource_not_modified(request, resource, **karg):
    format = _ERROR_CODE[_ERROR_CODE_RESOURCE_NOT_MODIFIED]
    message = format % (resource, karg.__repr__())
    return _write_error_response(HttpResponseNotModified, _ERROR_CODE_RESOURCE_NOT_MODIFIED, message)
...
def bad_data(request):
    ...
    return _write_error_response2(HttpResponseBadRequest, message, log_error=log_error)


def blacklisted_request(source, name, value):
    error = {
        'result': 'Refused', 'reason': 'One or more of the data item is blacklisted for the given source.',
        'source': source, 'name': name, 'value': value}
    message = simplejson.dumps(error)
    log_error = 'Blacklisted request: source=%s, name=%s, value=%s' % (
        source, name, value)
    return _write_error_response2(HttpResponseForbidden, message, log_error=log_error)


def exceeded_request(source, name, value, count):
    error = {
        'result': 'Exceeded', 'reason': 'One or more of the data item exceeds the limit for the given source.',
        'source': source, 'name': name, 'value': value, 'count': count}
    message = simplejson.dumps(error)
...
def empty_array_response(request):
    logger.warn('request:%s,empty array response' %
