                ...
                case 'init': //如果有需要验证框时初始化各项 
                    ...
                    need_confirm.each(function(){  //添加监听事件
                        addEvent(this.parentNode,'click',function(e){
                            ...
                            extra_com = /\bextra_com/.test(class_name)?node.getAttribute('extra_common'):'true';  //获取预先执行的扩展代码，主要用于push消息列表检测是否需要验证框
                            if(enable_break&&eval(extra_com)){
                                e.preventDefault();
                                e.stopPropagation(); //阻止默认事件及事件冒泡
                                tmp(e.target);  //调用本函数，并将当前事件传入
                            }else{
                                enable_break=true;  //验证成功重新触发点击，此次不弹框，但过后恢复
                            }
                            return;
                        }, true);
                    });
                    ...
                    addEvent(dom_arr[0],'click',arguments.callee.bind(this,'verification')); //添加验证事件，当用户点击确定后触发
                break;
                case 'show':  //显示验证框
                    ...
                    randoms=[Math.random(),Math.random()].sort(); //确保算式第一个比第二个大 防止减法出现负数
                    dom_arr[2].innerHTML = Math.floor(randoms[1]*10)+math_sign[Math.floor(randoms[0]*10%3)]+Math.floor(randoms[0]*10); //生成算式
                    dom_arr[0].style.display='block';
                break;
                case 'verification':   // 验证用户输入是否正确
                    switch(arguments[1].target.id){
                        case 'sure':
                            ...
                            if(target_dom){
                                enable_break = false;  // 验证成功，允许执行
                                target_dom.click(); // 重新触发事件
                                target_dom = null;
                            }
                            dom_arr[3].value = "";
                        break;
...
function push_method(){  // 消息推送列表强制预发布
    switch(arguments[0]){
        case 'init':  
            ...
            if(len>1){  //推送消息列表有数据
                for(var i=1;i<len;i++){
                    ...
                    if(tmp_value=='0'||(tmp_value==1&&tmp_sync=='需要同步')){
                        tmp_status.find('option').last().remove();  // 对于不能改为已发布的推送消息去掉已发布选项
                        disable_change.push('push_'+i);  // 记录符合这种情况的push消息，对于同步此类push消息不弹验证框
                    }
                }
            }
            sync_btn.each(function(){  // 对于在push消息列表的同步按钮默认需要验证框
        ...
        break;
        case 'verify':  // 检测是否需要弹出框 需要的情况是待同步列表里包含有正式发布的推送消息
...
function sync_to_line(){  // 异步同步需要同步的项
    ...
    (function(){
        var tmp;
        is_sent = false;  //  是否要发达同步请求
        sync_log=!arguments[0]?'没有传入调用类型！':sync_log;
        ajax_parameters=ajax_parameters?ajax_parameters:{  // 同步请求ajax参数
            url:window.location.href.replace(/\?.*/,'')+'syncto',
            data:{},
            timeout:180000,
            success:arguments.callee.bind(this,'repeat'), // 请求成功回调函数使用本函数 传入‘repeat’
            error:arguments.callee.bind(this,'error')  
        };
        switch(arguments[0]){
            case 'start':  // 初始化，
                tmp = $('.procesCover,#proflag,#showNum,#cancle_btn');
                if(tmp.length!=3){ // 如果不存在同步进度条dom则创建
                ...
                dom_arr=tmp;
                dom_arr[3].addEventListener('click',arguments.callee.bind(this,'cancle')); // 添加监听事件 
                ajax_parameters.data={'first':'true'}; // 首次发送同步请求加入特殊参数 此次只是返回未同步的数量，不同步
                is_sent = true;  
            break;
            case 'repeat':  //  未完成所有同步之前，重复请求
                if(cancle_flag){ // 点击取消 则取消
                    return ;
                }
                ...
                if(receive_data.all === undefined) {
                    ...
                    if(receive_data.suc === undefined){
                        ...
                        dom_arr[1].style.width = procesLen * (total_item - left_item) / total_item + 'px';  // 改变进度条进度
                    }
                    dom_arr[2].innerHTML = left_item + '/' + total_item; // 改变显示数据
                }
            break;
            case 'error':  // 请求错误处理函数 累计连续四级错误视为网络问题，退出，否则重新请求
                error_times++;
...
function changeSortDefault(){  // 改变列表内可排序项的首次排序方法为从大到小排
    ...
    if(ths[0].innerText.replace(/\s/g,'')==='展开详情'){ 
        [].shift.apply(ths); // 排除自定义的异步加载详情的干扰
    }
    if(/\?.*o=(\d+)/.test(location.href)){ 
        exclude = ths[RegExp.$1].children[0].innerText; // 若当前是已经对某一列排序，找到这一列
    }
    if(sort_th.length>0){
        sort_th.each(function(){
            if(exclude&&this.innerText == exclude){  // 不更改当前排序列的标记
                return ;
            }
            this.href=this.href.replace(/ot=asc/,'ot=desc');  // 更改默认排序方式
            this.parentNode.className='sorted ascending';  // 添加样式
        });
    }
    // for search bar
    ths.each(function(i){
        if(this.innerText.replace(/\s/g,'')=='版本'){  // 找到版本所有列
            version_order = i;
            return false;
        }
        return true;
...
function change_action(){ // 增加同步、删除按钮
    var action_div = $('.actions'), selectall_input = $('#action-toggle'),is_empty = selectall_input.length==0,top_prepend=[],bottom_pend=[],tmp;
    if(action_div.length == 1 && !is_empty){ // 排除没有同步或删除的情况
        var submit_btn = $('.actions .button'),
            action_select = $('.actions select[name="action"]'),
            action_options,
            options=[];
        if(action_select.length==1&&submit_btn.length==1){  // 根据有无同步、删除确定是否要添加
            ...
            action_options.each(function(i){
                var tmp_value = this.value,
                    tmp_match;
                if(typeof tmp_value=='string'&&(tmp_match=tmp_value.match(/upload|delete|export/))){
                    options.push({id:i,type:tmp_match[0],text:this.innerText}); // 用于生成按钮参数
                }
            });
            if(options.length==0){
                return ;
            ...
            submit_btn.hide();
            //tmp = options[1]?'<button class="'+options[0].type+'_btn">'+options[0].text+'</button><button class="'+options[1].type+'_btn">'+options[1].text+'</button>':'<button class="'+options[0].type+'_btn">'+options[0].text+'</button>';  // 定义按钮的位置
            tmp = options.map(function(item){return '<button id="'+item.id+'" class="'+item.type+'_btn">'+item.text+'</button>'}).join('')
            top_prepend.push(tmp);
            bottom_pend.push(tmp);
            action_div.prepend(top_prepend.join(''));
            $('#changelist-footbar').prepend('<div class="actions bottomact">'+bottom_pend.join('')+'</div>');
            $('.actions').bind('click',function(e){  // 监听按钮事件，模拟原生事件
                try{
                    ...
                    btn_id = parseInt(target.id)
                    if(target && class_name && btn_id != NaN){
                        match = class_name.match(/selectall|upload|delete|export/)
                        switch(match?match[0]:null){
                        //case 'selectall':
                        ...
                        //break;
                            ...
                            case 'upload':
                            case 'delete':
                                action_select[0].selectedIndex = btn_id;
                                submit_btn.click();
                            break;
                            default:
                            break;
...
function event_bind(){
    /*$('.vForeignKeyRawIdAdminField.customForeignKey').change(function() {// 原生
        var value = $(this).val();
    ...
    $(".vIntegerField").keydown(function(event) {// 原生
    ...
    jQuery('#leftnav h3, #changelist-filter h3').click(function() {// 左导收起展开
            ...
            case 'init': // 初始化 隐藏input 请求api组织所有的标签选项
                ...
                $.getJSON('/get_app_labels',function(reData){
                    if(Object.prototype.toString.call(reData)==='[object Array]'){
                        all_tags = reData;
                        parent_node.append(all_tags.map(self.bind(this,['build_html','one_tag'])).join('')); //插入页面中，让浏览器渲染
                        if(tag_str){
                ...
                switch(method[1]){
                    ...
                    case 'one_tag': 
                        if(array_item&&array_item.key){  
                            ...
                            returns='<div class="tags" id="'+array_index+'">'+array_item.name+':<span class="first_tag" ><span class="tag_tit" >请选择标签</span><ul class="tag_list '+array_item.select_mode+'" >'+array_item.tags.map(self.bind(this,['build_html','one_tag_item'])).join('')+'</ul></span><span class="sec_tag" id="sec_tag">'+sec_tag_doms.join('')+'</span></div>';// 生成单个应用标签包含一级，二级
                            sec_tag_doms = [];
                            return returns;
                        }
                    break;
                    case 'one_tag_item': // 生成一级选项
                        if(Object.prototype.toString.call(array_item)==='[object Array]'){  // 包含二级的一级标签 生成二级，并在select_arr 中留下相应的记录
                            ...
                            select_arr[select_arr.length-1].sec_tag[array_index]=sec_tags.map(function(item){return{name:item,select:true};})//设置二级标签初始状态为已选 
                        }else{
                    ...
                    case 'sec_tag_item': // 生成二级选项
                        return typeof(array_item) =='string'?'<span class="sec_item on" id='+array_index+'>'+array_item+'</span>':'';
                    break;
                    default:
                    break;
                }
            ...
            case 'action':
                ...
                switch(match&&match[0]){
                    ...
                    case 'tag_item': // 点击的是一级标签
                        ...
                        if(is_multi){
                            if(/selected/.test(current_class)){ // 多选一级存成一个数组
                    ...
                    for(item in current_tag.sec_tag){  //初始化二级标签的参数
                        ...
                        if(current_tag.sec_tag.hasOwnProperty(item)){
                            jqery_dom.find('#tag_'+one_tag_pos[item]+',#tag_'+one_tag_pos[item]+' >span').each(function(){
                                ...
                                if(sec_tags.indexOf(this.innerText.replace(/\s/g,''))==-1){
...
function match_feed_back(){  // 卸载反馈导出execl
    var tmp = $('#content h1');
    if (tmp&&tmp[0]&&tmp[0].innerText.indexOf('用户反馈')!=-1){
        $('.results #result_list').css('tableLayout','fixed')
        $('#changelist #toolbar').append('<form id="excel_form" action="/promotion/feedback.xls" method="get"><input type="submit" value="导出Excel文档" style="position: absolute;top: 15px;left: 14px;height: 26px;color: #000;background: #f2f2f2;"></form>');
    }
    return ;
}
...
window.addEventListener('load', function(){
    ...
    if(tmp_save_btn.length==1){  // 复制保存按钮
