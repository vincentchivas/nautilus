...
class AppList(LeftNavItem, AppListElementMixin):
    ...
    def init_with_context(self, context):
        ...
        include_labels = self.app_label_order.keys()
        apps = {}
        for model, perms in items:
            if not perms['change']:
                continue
            app_label = model._meta.app_label
            if app_label not in include_labels:
                continue
...
class ReportingItem(AppList):
    ...
    def __init__(self, items, title=None, **kwargs):
        """
    ...
    def init_with_context(self, context):
        ...
        for url, reporting_name, title in self.items:
