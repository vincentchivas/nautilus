...
Advertise.prototype = {
	...
	initAdvertise : function(){
		var tempThis = this;
		var advsList = null;
		//var url = "http://172.16.9.151/api/1/adverts.json?cid=" + this.cid;
		//var url = "/api/1/adverts.json?cid=8";
		var url = "/api/1/adverts.json?cid=" + this.cid;
		...
		/*if(!this.getSLocal("advs") || !(advsList = JSON.parse(this.getSLocal("advs")))){
			this.ajaxGet(encodeURI(url),function(text){tempThis.manageAdvs(text);},function(text){tempThis.manageAdvs(text);});
		}else{
	...
	showAdvs : function(data){
		if(this.advsdisplay){
			//已经显示了广告,直接退出.
			alert("advs shown, directly exit!");
			return true;
		}
		...
		for(var i=0;i<data.length;i++){
			if(data[i].mode == "imag"){
				...
				//var pos = this.strategyPicture(data[i].tweight,pos_data);
				//取得要显示图片广告的位置，建立一个IMG元素，并将选定的图片url赋值给img的src。
				dom = this.advsPos[data[i].iid];
				if(!dom){
					continue;
				}
				...
				switch (data[i].iid) {
					case 27:
					document.getElementsByClassName("recommend_nove_des")[0].href = rdata[0].url;
					dom.getElementsByTagName("h1")[0].innerHTML = rdata[0].title;
					dom.getElementsByClassName("author")[0].innerHTML = rdata[0].author;
					dom.getElementsByTagName("p")[0].innerHTML = rdata[0].summary;
					dom.getElementsByClassName("floatImg")[0].src = rdata[0].images[0];
					...
					for(var j=0; j <= rdata.length-1;j++){
							document.getElementsByTagName("a")[j+5].href = rdata[j].url;
							document.getElementsByTagName("img")[j+1].src = rdata[j].images[0];
							document.getElementsByClassName("coverTitle")[j].innerHTML = rdata[j].title;
					}
					dom.style["display"] = "block";
					break;
				}
		...
		if(cycleShow.length > 0){
			...
			for(var i=0;i<cycleShow.length;i++){
				...
				rdata = cycleShow[i].data;
				for(var j=0; j <= rdata.length-1;j++){
					document.getElementsByClassName("recoUrl")[j].href = rdata[j].url;
					document.getElementsByClassName("recoTitle", dom)[j].innerHTML = rdata[j].title;
				}
				dom.style["display"] = "block";
			}
		}
		
		function autoText(thisTag, needLen) {
			var idid = document.getElementsByTagName(thisTag)[0];
			var nowLen = idid.innerHTML.length;
			if (nowLen > needLen) {
				var nowWord = idid.innerHTML.substr(0, needLen) + '...';
				idid.innerHTML = nowWord;
			}
		}
