...
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/var/app/enabled/dolphinop-service
DAEMON=/usr/local/bin/uwsgi
NAME=dolphinop
DESC="Dolphin Operation Service uWSGI Web Server"
PROJECT=dolphinop-service
SPIDERS=1
...
case "$1" in
	...
	status)
		status_of_proc -p /var/run/dolphinop-service-uwsgi.pid "$DAEMON" uwsgi && exit 0 || exit $?
		;;
	*)
