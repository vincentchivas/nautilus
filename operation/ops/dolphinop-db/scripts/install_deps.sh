...
function add_mongodb_repository()
{
    APT_SOURCES=/etc/apt/sources.list
    MONGODB_SOURCE="deb http://downloads-distro.mongodb.org/repo/ubuntu-upstart dist 10gen"

    if [ "`cat ${APT_SOURCES} | grep -c '${MONGODB_SOURCE}'`" = 0 ]; then
        echo ${MONGODB_SOURCE} >> ${APT_SOURCES}
...
function install_mongodb()
{
    ...
    if [ `aptitude  search  mongodb-clients  | grep -c "^i \+${1} \+"` = 0 ];then
        aptitude remove mongodb-clients
    fi
    install_sys_dep mongodb-10gen
}
