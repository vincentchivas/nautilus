var baina = {
	...
	errorPic : function(pic,bgcolor){
		...
		if(bgcolor){
			...
			pNode.style["background"] = "#d0d0d0 url(images/dolphin_b.png) no-repeat center center";
		}
	...
	track : function(action_type,item_type,item_self,reference_item_type,reference_item,position,no_delay){
		//用于统计
		if(navigator.onLine){
			//有网的情况下才统计
...
baina.touchSet.prototype = {
	...
	_TouchMove : function(e){
		...
		var acceleration = this.speedY > 0 ? -0.004 : 0.004;//加速度常量，可根据喜好适当调整。减速运动，所以方向跟速度是相反的。
	...
	_TouchEnd : function(e){
		...
		if(this.cY>0 || Math.abs(this.cY)>this.availbleH){
			...
			if(this.borderStartY){
				...
				this.autoMoveTo(this.tTarget,0,this.cY,200,"ease");
			}else{
	...
	setcy : function(cy){
		//给外部使用的，设置滑动到的位置
		this.cY = cy;
		baina.setTransform(this.tTarget,0,0);
	}
};
