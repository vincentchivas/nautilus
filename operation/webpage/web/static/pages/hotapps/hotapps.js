...
/***************************************app update info*************************************/
	...
	pageTabInit: function() {		//tab页面切换初始化
		...
		baina.addEvent(baina.$('backFromSearch'), 'click', function (e) {
			baina.$('searchForm').style.display = 'none';
			baina.$('search').value = '';
		...
		baina.addEvent(baina.$('searchBtn'), 'click', function() {location.hash = 'search';});
...
//根据API生成页面内容,无app信息的时候还是需要显示推荐的App。
	...
	mainTabChange: function() {
		...
		baina.$('searchBtn').style.display = 'block';
		...
		switch(switchNum){
			...
			case 'search':
							...
							baina.$('searchBtn').style.display = 'none';
							baina.$('updateEntry').style.display = 'none';
							baina.$('search').value = '';
							baina.$('word').style.display = 'block';
							...
							baina.$('searchForm').style.display = 'block';
