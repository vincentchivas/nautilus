...
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/var/app/enabled/provider-service
...
function start_uwsgi()
{
	if [ -f "$PID_FILE" ]; then
	    ...
	    pushd ${APP_DIR}/provider >/dev/null
...
case "$1" in
	...
	status)
		status_of_proc -p /var/run/provider-service-uwsgi.pid "$DAEMON" uwsgi && exit 0 || exit $?
		;;
	*)
